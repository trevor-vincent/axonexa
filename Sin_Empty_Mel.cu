...
int main (){
  ...
  phaseAcquisition<SinGFunc> pa(numOfPeriods.size(),number_of_timesteps,number_of_particles,time(NULL));
  ...
  for (int j = 0; j < numOfPeriods.size(); j++){
		...
		SinGFunc cosGRAD(G, gradient_duration,gradient_spacing, N, Vector3(1.0,0.0,0.0));
