...
public:
  ...
  phaseAcquisition(int _numOfMeas, int _numOfSteps, int _numOfParticles, int _seed){
    ADCmeansquare = 0;
	points = 0;
  ...
  std::vector<FieldType> & getGradientFunctors(){
    return gradientFunctors;
  }
	
  std::vector<real> & getMx(){
    return mx;
  }

  std::vector<real> & getMy(){
    return my;
  }
  
  real getSignal(int i){
	return sqrt(mx[i]*mx[i] + my[i]*my[i]);
  }
  ...
  void calcADC(){
	...
	if (points == 1){	
		...
		ADCmeansquare = (ADCmeansquare*(points-1) + ADCtemp*ADCtemp)/(points);
	}
...
real & getADCMeanSquare(){
	return ADCmeansquare;
}
...
real getADCError(){
	return sqrt( ADCmeansquare - ADC*ADC );
}
