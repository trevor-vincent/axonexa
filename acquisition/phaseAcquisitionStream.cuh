...
public:
  ...
  phaseAcquisitionStream(int num){number_of_particles = num; steps_max = -1; meas_max = -1;}
  ...
  phaseAcquisition<FieldType> & getAcquisition(int num){
  ...
  void runAcquisition(int num, Basis & basis, real timestep, int blocks, int threads, int numOfSM){
    ...
    cudaVector<FieldType> dev_fields(acqSet[num].getGradientFunctors().size());
    ...
    cudaScalar<SimuParamsPhase> dev_par( SimuParamsPhase(number_of_particles,
							 steps,
							 measurements, 
							 timestep, 
							 acqSet[num].getSeed(),
    ...
    dev_phase.transformAndSum<COS_UNARY<real> >(dev_total_mx, 1024, numOfSM, measurements, number_of_particles, 0);
    dev_phase.transformAndSum<SIN_UNARY<real> >(dev_total_my, 1024, numOfSM, measurements, number_of_particles, 0);
  ...
  void runAcquisitionCPU(int num, Basis & basis, real timestep){
    ...
    SimuParamsPhase pars(number_of_particles,
			 steps,
			 measurements, 
			 timestep, 
			 acqSet[num].getSeed(),
    ...
    updateWalkersPhaseCPU<FieldType, Basis>(&pars, &basis, &acqSet[num].getGradientFunctors()[0], &phase[0]);
  ...
  template <class Basis>
  void runAcquisitionLattice(int num, Basis* basis, Lattice & lat, real timestep, int blocks, int threads, int numOfSM){
    ...
    updateWalkersLatticePhase<FieldType, Basis> <<< blocks, threads >>> (
									 ...
									 dev_fields.getPointer(), 
									 dev_states.getPointer(),
									 dev_T2factors.getPointer(),
									 dev_phase.getPointer()
									
									 );	

#if defined USE_RELAXATION									
    dev_phase.transformAndSumTwoVectors<COS_EXP_BINARY<real> >(dev_total_mx, dev_T2factors, 1024, numOfSM, measurements, number_of_particles, 0);
    dev_phase.transformAndSumTwoVectors<SIN_EXP_BINARY<real> >(dev_total_my, dev_T2factors, 1024, numOfSM, measurements, number_of_particles, 0);
#else
    dev_phase.transformAndSum<COS_UNARY<real> >(dev_total_mx, 1024, numOfSM, measurements, number_of_particles, 0);
    dev_phase.transformAndSum<SIN_UNARY<real> >(dev_total_my, 1024, numOfSM, measurements, number_of_particles, 0);
#endif
  ...
  template <class Basis>
  void runAcquisitionWC(int num, Basis & basis, real timestep, int blocks, int threads){
	...
	dev_Gx[j] = acqSet[num].getGradientFunctors()[i](j*timestep).x;
	dev_Gy[j] = acqSet[num].getGradientFunctors()[i](j*timestep).y;
	dev_Gz[j] = acqSet[num].getGradientFunctors()[i](j*timestep).z;
      }
      ...
      acqSet[num].getMx()[i] = computePhaseSx(number_of_particles, dev_phase.getPointer(), 1);
      acqSet[num].getMy()[i] = computePhaseSx(number_of_particles, dev_phase.getPointer(), 0);
  ...
  void runAcquisitionStream(
    ...
    std::vector< std::vector< cudaScalar<SimuParamsPhase> > > dev_pars(numOfDevices, std::vector< cudaScalar< SimuParamsPhase > >(2) );
    std::vector< std::vector< cudaScalar<Basis> > > dev_basis(numOfDevices, std::vector< cudaScalar< Basis > >(2) );
	
    std::vector< std::vector< cudaVector<real> > >dev_phase(numOfDevices,std::vector< cudaVector<real> >(2));
	...
	host_pars[i] = SimuParamsPhase(number_of_particles,
				       steps,
				       measurements, 
				       timestep, 
				       acqSet[i].getSeed(),
	...
	host_pars[i+1] = SimuParamsPhase(number_of_particles,
					 steps1,
					 measurements1, 
					 timestep, 
					 acqSet[i+1].getSeed(),
	...
	cudaStreamQuery(streams[devNum][0]);	 
	cudaStreamQuery(streams[devNum][1]);
	updateWalkersPhase<<<blocks, threads, 0, streams[devNum][0]>>> (dev_pars[devNum][0].getPointer(), dev_basis[devNum][0].getPointer(), dev_fields[devNum][0].getPointer(), devStates[devNum][0].getPointer(), dev_phase[devNum][0].getPointer() ) ; 
	updateWalkersPhase<<<blocks, threads, 0, streams[devNum][1]>>> (dev_pars[devNum][1].getPointer(), dev_basis[devNum][1].getPointer(), dev_fields[devNum][1].getPointer(), devStates[devNum][1].getPointer(), dev_phase[devNum][1].getPointer() );
	cudaStreamQuery(streams[devNum][0]);	 
	cudaStreamQuery(streams[devNum][1]);	
	// #if defined USE_RELAXATION		
	// dev_phase[devNum][0].transformAndSumTwoVectors<SIN_EXP_BINARY<real> >(dev_total_my[devNum][0], dev_T2factors[devNum][0], 1024, numOfSM, measurements, number_of_particles, streams[devNum][0]);
	// dev_phase[devNum][1].transformAndSumTwoVectors<SIN_EXP_BINARY<real> >(dev_total_my[devNum][1], dev_T2factors[devNum][1], 1024, numOfSM, measurements, number_of_particles, streams[devNum][1]);
	// dev_phase[devNum][0].transformAndSumTwoVectors<COS_EXP_BINARY<real> >(dev_total_mx[devNum][0], dev_T2factors[devNum][0],  1024, numOfSM, measurements, number_of_particles, streams[devNum][0]);
	// dev_phase[devNum][1].transformAndSumTwoVectors<COS_EXP_BINARY<real>>(dev_total_mx[devNum][1], dev_T2factors[devNum][1], 1024, numOfSM, measurements, number_of_particles, streams[devNum][1]);
	// #else
	dev_phase[devNum][0].transformAndSum<SIN_UNARY<real> >(dev_total_my[devNum][0], 1024, numOfSM, measurements, number_of_particles, streams[devNum][0]);
	dev_phase[devNum][1].transformAndSum<SIN_UNARY<real> >(dev_total_my[devNum][1], 1024, numOfSM, measurements, number_of_particles, streams[devNum][1]);
	dev_phase[devNum][0].transformAndSum<COS_UNARY<real> >(dev_total_mx[devNum][0], 1024, numOfSM, measurements, number_of_particles, streams[devNum][0]);
	dev_phase[devNum][1].transformAndSum<COS_UNARY<real> >(dev_total_mx[devNum][1], 1024, numOfSM, measurements, number_of_particles, streams[devNum][1]);
	// #endif
	cudaStreamQuery(streams[devNum][0]);	 
	cudaStreamQuery(streams[devNum][1]);	
  ...
  template <class Basis>
  void runAcquisitionStreamLattice(
				   Basis * basis, 
				   Lattice & lat,
				   real timestep, 
				   int blocks, 
    ...
    std::vector< std::vector< cudaVector< FieldType > > > dev_fields(numOfDevices,std::vector< cudaVector< FieldType > >(2) );
    std::vector< std::vector< cudaScalar<SimuParamsPhase> > > dev_pars(numOfDevices, std::vector< cudaScalar< SimuParamsPhase > >(2) );
    std::vector< std::vector< cudaScalar<Lattice> > > dev_lat(numOfDevices, std::vector< cudaScalar< Lattice > >(2) );
    std::vector< std::vector< cudaVector<Basis> > > dev_basis(numOfDevices, std::vector< cudaVector< Basis > >(2) );
	
    std::vector< std::vector< cudaVector<real> > >dev_phase(numOfDevices,std::vector< cudaVector<real> >(2));
    std::vector< std::vector< cudaVector<real> > >dev_T2factors(numOfDevices,std::vector< cudaVector<real> >(2));
    std::vector< std::vector< cudaVector<real> > >dev_total_mx(numOfDevices,std::vector< cudaVector<real> >(2));
    std::vector< std::vector< cudaVector<real> > >dev_total_my(numOfDevices,std::vector< cudaVector<real> >(2));
	...
	updateWalkersLatticePhase<<<blocks, threads, 0, streams[devNum][0]>>> (dev_pars[devNum][0].getPointer(), dev_lat[devNum][0].getPointer(), dev_basis[devNum][0].getPointer(), dev_fields[devNum][0].getPointer(), devStates[devNum][0].getPointer(), dev_T2factors[devNum][0].getPointer(), dev_phase[devNum][0].getPointer() );
	updateWalkersLatticePhase<<<blocks, threads, 0, streams[devNum][1]>>> (dev_pars[devNum][1].getPointer(), dev_lat[devNum][1].getPointer(), dev_basis[devNum][1].getPointer(), dev_fields[devNum][1].getPointer(), devStates[devNum][1].getPointer(), dev_T2factors[devNum][1].getPointer(), dev_phase[devNum][1].getPointer() );
    ...
    for (int devNum = 0; devNum < numOfDevices; devNum++){
      safe_cuda(cudaSetDevice(devNum));	//ADDED THIS RECENTLY (MARCH 4th 2013)
      cudaStreamSynchronize( streams[devNum][0] );
      cudaStreamSynchronize( streams[devNum][1] );
      cudaStreamDestroy( streams[devNum][0] );
      cudaStreamDestroy( streams[devNum][1] );
  ...
  void addMagnetizationSet(vector<pinnedVector<real> > & host_mx, vector<pinnedVector<real> > & host_my){
    for (int i = 0; i < acqSet.size(); i++){
