...
double ds; //diffusion coefficient of the sheath, atm this is NOT BEING USED by any of the code
double dc; //diffusion coefficient of the core, atm this is NOT BEING USED by any of the code
double rs; //radius of the sheath, atm this is NOT BEING USED by any of the code
double rc; //radius of the core, atm this is NOT BEING USED by any of the code
double T2e; //T2 extracellular
double T2i; //T2 intracellular
double T2s; //T2 for the sheath, atm this is NOT BEING USED by any of the code
double T2c; //T2 for the core, atm this is NOT BEING USED by any of the code
...
double csz; //z dimension cell size for szafer box cells (NOT BEING USED BY ANYTHING). But I would still recommend setting csz = ls, just in case it is being used
double p; //permeability
double dt; //timestep
double ls; //lattice size
double g; //initial gradient strength
double hex_sidelength; //hexagon sidelength d (see lattice.h for more info about this)
bool printparticleinfo; //prints particle info, NOT SUPPORTED BY THE CODE ANYMORE
bool printparticleposition; //prints the trajectories of the particles to fiel
...
int graddir; //direction of the gradient (0 = x-axis, 1 = y-axis, 2= z-axis)
int num_of_trial; //number of different b values used for linear regression calculation
int num_of_repeat; //number of times to repeat the simulation (back to back simulations with the same initial parameters)
...
int cyl_seed; //seed for random number generator used to generate cylinder positions and radii
double rand_mean;
double rand_stddev;
...
/* Here we connect the parameter (e.g. geom) with a shortcut character (e.g. 'a') */
...
/* This is the menu. It comes up if all you type in the command line is the "executable name" (e.g. test6) */
if (argc == 1){ //if arguments == 1, i.e. only the executable name has been typed.
std::cout << "Monte Carlo Simulation of Diffusion with restricting geometries" << std::endl << std::endl;
std::cout << "Usage: test [options] -geom <geometry packing> -num <number of particles> " << std::endl;
std::cout << "       test -geom <geometry packing> -num <number of particles> [options] " << std::endl << std::endl << std::endl;
std::cout << "Available Options are: " << std::endl << std::endl;
std::cout << "       -geom <geometry packing>                         (0 = Szafer Box Lattice, 1 = Cylinder Box Lattice, no default)" << std::endl;
std::cout << "       -num  <number of particles>                      (no default)" << std::endl;
std::cout << "       -de <Extracellular Diffusion Coefficient>        (geometry 0,1 default = 2.5e-6 mm^2/ms)" << std::endl;
std::cout << "       -di <Intracellular Diffusion Coefficient>        (geometry 0 default = 1.0e-6 mm^2/ms)"<< std::endl;
std::cout << "       -T2e <Extracellular T2>                          (geometry 0,1 default = 200 ms)" << std::endl;
std::cout << "       -T2i <Intracellular T2>                          (geometry 0 default = 70 ms)"<< std::endl;   
std::cout << "       -radius <Cell Cylinder Radius>                   (geometry 1 default = 0 mm)"<< std::endl;                         
std::cout << "       -gt <gradient time>                              (geometry 0,1 default = 1 ms)" << std::endl;
std::cout << "       -gs <gradient spacing>                           (if dtg = 0, this is the diffusion time. geometry 0,1 default = 10 ms)" << std::endl;
std::cout << "       -csx <Cell Size in x dimension>                  (geometry 0 default = 0)" << std::endl;
std::cout << "       -csy <Cell Size in y dimension>                  (geometry 0 default = 0)" << std::endl;
std::cout << "       -perm <permeability>                             (geometry 0,1 default =  .005)" << std::endl;
std::cout << "       -dt <timestep>                                   (geometry 0,1 default =  .1 ms)" << std::endl;
std::cout << "       -ls <lattice size>                               (geometry 0,1 default = 1 mm) "<< std::endl;
std::cout << "       -grad <initial gradient strength>                (geometry 0,1 default = 0.0001609114 T/mm)" << std::endl;
std::cout << "       -help                                            (not available atm)                            " << std::endl;       
std::cout << "       -dim <dimension>                                 (default = 3, no other dimensions are supported at this time)" << std::endl;
std::cout << "       -printparticleposition                           (default = false)" << std::endl;
std::cout << "       -printsignalinfo                                 (default = false)" << std::endl;
std::cout << "       -printinitialvars                                (default = false)" << std::endl;
std::cout << "       -printtofile <text file>                         (default = false, prints (timestep, ADC) for graddir to text file)" << std::endl;
std::cout << "       -graddir <gradient direction>                    (default = 0(x axes), 1(y axes), 2(z axes)" << std::endl;
std::cout << "       -gaussian <text file>                            (default = false) " << std::endl;
std::cout << "       -increments <number of times ADC is computed>    (default = 1) " << std::endl;
std::cout << "       -hex <hexagonal sidelength>                      (default = .5) " << std::endl;
std::cout << "       -dontuseT2                                       (default = true) " << std::endl;
std::cout << "       -num_of_trial                                    (default = true) " << std::endl;
std::cout << "       -num_of_repeat                                   (default = 1) " << std::endl;
std::cout << "       -num_of_cyl                                      (geom 3 default = 1) " << std::endl;
std::cout << "       -max_radius                                      (geom 3 default = no default) " << std::endl;
std::cout << "       -min_radius                                      (geom 3 default = no default) " << std::endl;
std::cout << "       -cyl_seed                                        (geom 3 default = 12345678.12345678) " << std::endl;
exit (1);
}


/* 
	if more than just the executable name is typed, this else block is executed
	it contains the instructions to be executed if one of the parameters is selected
	Usually the instructions just place the command line user-entered parameter
	into the variables struct under the corresponding name.
 */
else {
	...
	if(count != 2){std::cout << "ERROR: both -geom and -num parameters are required" << std::endl; exit(1);}
}
