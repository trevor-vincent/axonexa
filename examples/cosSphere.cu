...
Sphere spher1(0.0,0.0,0.0,rad1,0.0,0.0,D,0,0.0);
Sphere spher2(0.0,0.0,0.0,rad2,0.0,0.0,D,0,0.0);
Sphere spher3(0.0,0.0,0.0,rad3,0.0,0.0,D,0,0.0);
Sphere spher4(0.0,0.0,0.0,rad4,0.0,0.0,D,0,0.0);
 ...
 for (int i = 0 ; i < 3; i++){
 pas.runAcquisitionStream(spher1, timestep, blocks, threads, 1, plan, numOfSMPerDevice); 
 pas.calcEveryADC();
 ...
 for (int i = 0; i < NOI; i++){
  ...
  std::cout << " NEXT SPHERE " << std::endl;

 for (int i = 0 ; i < 3; i++){
 pas.runAcquisitionStream(spher2, timestep, blocks, threads, 1, plan, numOfSMPerDevice); 
 pas.calcEveryADC();
 pas.changeSeeds(time(NULL)*(i+1));
 }
 std::cout << std::endl << std::endl;
 ...
 for (int i = 0 ; i < 3; i++){
 pas.runAcquisitionStream(spher3, timestep, blocks, threads, 1, plan, numOfSMPerDevice); 
 pas.calcEveryADC();
 pas.changeSeeds(time(NULL)*(i+1));
 }
 std::cout << std::endl << std::endl;
 ...
 for (int i = 0 ; i < 3; i++){
 pas.runAcquisitionStream(spher4, timestep, blocks, threads, 1, plan, numOfSMPerDevice); 
 pas.calcEveryADC();
 pas.changeSeeds(time(NULL)*(i+1));
 }
