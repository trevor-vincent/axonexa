...
int main (){
  ...
  int NOM = 2;
	...
	for (int j = 0; j < NOI; j++){
		...
		int number_of_timesteps = (int) (echo_time/timestep);		
		magAcquisition<QSC_COS_GRE> pa(NOM,number_of_timesteps,j*time(NULL));
		...
		for (int i = 0; i < NOM; i++) {
			...
			QSC_COS_GRE qscCOS(B0,G, gradient_duration,gradient_spacing, N);
...
ctime.start();
for(int i = 0; i < 3; i++){
 pas.runAcquisitionStream(spher3,initialM, timestep, blocks, threads, 1, plan, numOfSMPerDevice); 
 pas.calcEveryADC();
