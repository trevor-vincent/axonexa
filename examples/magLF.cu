    ...
    for (int r1 = 1; r1 < repeats1 + 1; r1++) {
        for (int r2 = 1; r2 < repeats2 + 1; r2++) {
            ...
            real gradient_spacing =
                r2 * .1; // echo_time - 2.0*gradient_duration;
            ...
            for (int i = 0; i < numOfAcq; i++) {
                ...
                int number_of_timesteps = (int)(echo_time / timestep);
                QSC_GRE qsc(B0, i * .000006 / ((real)numOfAcq),
                            gradient_duration, gradient_spacing);
                magAcquisition<QSC_GRE> pa(1, number_of_timesteps,
                                           i * time(NULL));
                pa.addMeasurement(qsc);
                pas.addAcquisition(pa);
            }
            ...
            pas.runAcquisitionStream(cylinder, initialM, timestep, blocks,
