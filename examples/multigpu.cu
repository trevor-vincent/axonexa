...
int main() {
    ...
    for (int j = 0; j < NOI; j++) {
        ...
        // std::cout << "number_of_timesteps = " << number_of_timesteps <<
        // std::endl;
    ...
    std::vector<int> numOfSMPerDevice(2);
    ...
    plan[0] = 0;
    plan[1] = NOI - 20;
    plan[2] = NOI; // NOI-14 is best for float //NOI-10 is best for double
    timer1.start();
    pas.runAcquisitionStream(spher, timestep, blocks, threads, 2, plan,
                             numOfSMPerDevice);
    timer1.stop();
    timer1.display();
    pas.flushADC();
