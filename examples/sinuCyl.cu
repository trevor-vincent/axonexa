...
int main() {
    ...
    for (int j = 0; j < NOI; j++) {
        ...
        for (int i = 0; i < NOM; i++) {
            ...
            COS_GRE cosGRAD(0.1, G, Vector3(1.0, 0.0, 0.0), gradient_duration,
                            gradient_spacing, N);
    ...
    pas.runAcquisitionStream(cylinder, sub, initialM, timestep, blocks, threads,
                             2, plan, numOfSMPerDevice);

    // pas.runAcquisition(0, cylinder,  sub, initialM,  timestep, blocks,
    // threads, 1 , 2); pas.runAcquisition(1, cylinder,  sub, initialM,
    // timestep, blocks, threads, 1 , 2); pas.runAcquisition(2, cylinder,  sub,
    // initialM,  timestep, blocks, threads, 0 , 14); pas.runAcquisition(3,
    // cylinder,  sub, initialM,  timestep, blocks, threads, 0 , 14);

    for (int i = 0; i < NOI; i++) {
        // std::cout << " ERROR HERE " << std::endl;
        // cout << Vector3( pas.getAcquisition(i).getMx()[0],
        // pas.getAcquisition(i).getMy()[0],
        // pas.getAcquisition(i).getMz()[0])*(1.0/number_of_particles);
