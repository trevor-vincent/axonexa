...
int main() {
    ...
    // 0.0, D_extra,4); cyls[0] = Cylinder_XY(d/2.0, 0.0,  radius,  T2_i,0.0,
    // D_intra, 1,0.0); cyls[1] = Cylinder_XY(0.0,
    // d*0.86602540378443864676372317075294,  radius,  T2_i,0.0, D_intra,
    // 2,0.0); cyls[2] =
    // Cylinder_XY(d/2.0, 2.0*d*0.86602540378443864676372317075294,  radius,
    // T2_i,0.0, D_intra, 3,0.0); cyls[3] = Cylinder_XY(d,
    // d*0.86602540378443864676372317075294,  radius,  T2_i,0.0, D_intra,
    // 4,0.0);
    ...
    for (int i = 0; i < 16; i++) {
        ...
        cyls[0] =
            Cylinder_XY(a / 2.0, a / 2.0, radius, T2_i, 0.0, D_intra, 1, 0.0);
        pas.runAcquisitionLattice(i, &cyls[0], lattice, timestep, blocks,
                                  threads, 14);
