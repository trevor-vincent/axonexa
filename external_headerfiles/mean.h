	...
	This header file contains some very useful subroutines used throughout the code which includes:
	...
	*/
	...
	double mean(double nums[], int number){
		...
		double mean = 0.0;
		for (int i = 0; i<number; i++){mean = mean + nums[i];}
		mean = mean/number;
		return mean;
	}
	...
	double mean_square(double nums[], int number){
		double mean = 0.0;
		for (int i = 0; i<number; i++){mean = mean + nums[i]*nums[i];}
		mean = mean/number;
		return mean;
	}
	...
	double std_dev(double nums [], int number ){
		...
		double std_dev = 0.0;
		for (int i = 0; i < number; i++){std_dev = std_dev + (nums[i] - mean_num)*(nums[i] - mean_num)/number;}
	...
	/***************************************************************************************
		...
		Linear regression a la Bevington. He includes the uncertainty in yi (sigma_i) into the calculation of the slope.
		term 1 -> sum(1/sigma_i^2)
		term 2 -> sum(yi*xi/sigma_i^2)
		term 3 -> sum(xi/sigma_i^2)
		term 4 -> sum(yi/sigma_i^2)
		term 5 -> sum(xi^2/sigma_i^2)

		returns slope
	 ****************************************************************************************/	

	double linear_regression(double y [], double x [], int number, double sigma [], double *error){
		
		double term1 = 0.0, term2 = 0.0, term3 = 0.0, term4 = 0.0, term5 = 0.0, delta;
	...
	/***************************************************************************************
		...
		Linear regression without the uncertainty in y. This makes the vital assumption that the uncertainty in the yi's is CONSTANT. 
		They should, however, be constant for us.
	 ****************************************************************************************/	
	 
	double linear_regression(double y [], double x [], int number){
		
		double term1 = number, term2 = 0.0, term3 = 0.0, term4 = 0.0, term5 = 0.0, delta;
	...
	/***************************************************************************************
		...
		y(x1,x2,x3,x4,x5,x6) = a1 + a2*x1 + a3*x2 + a4*x3 + a5*x4 + a6*x5 + a7*x6
		
		the numerical recipes SVDFIT requires a function which defines y without the parameters
		a1->a7 and this is it.
	 ****************************************************************************************/	
	...
	double multi_lin( double lnsignal[], double b[][6], int num , double D[]){
		...
		MatDoub xx(num,6); //n x 6 matrix where n is the number of b vectors and 6 is the b components 
	...
	/***************************************************************************************
		...
		Calculates the average and the std deviation of the adcs for back to back simulations
	
	 ****************************************************************************************/	
	void meanADC (double ADC [], int num_of_inc, int num_of_repeat, double ADC_avg [], double ADC_error []) {
	...
	void meanADC (const vector<double> & ADC, int num_of_inc, int num_of_repeat, vector<double> & ADC_avg , vector<double> & ADC_error) {
	...
	// void meanADC (vector <MeasurementDB<SEQUENCE> > MDB_array, double & mean, double & stddev) {
	...
	bool memberofarray(double num, double array [], int size_of_array)
	{
		...
		for (int i = 0; i < size_of_array; i++) {
			...
			if ( doub_equal(num , array[i]) == true) {return true;}
