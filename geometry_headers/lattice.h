	...
	#ifndef LATTICE_H_ //only allows one class declaration memory allotment to be made
	#define LATTICE_H_
	...
	// Lattice(double _D_extra, double _T2_extra, double _permeability, int _number_of_elements){
	...
	FILE_LOG(logINFO) << " PARTICLES UNIFORMLY DISTRIBUTED ABOUT SUBSTRATE" << endl;
	Point scoords;
	double a0,b0,c0;
	...
	//note that this method assumes intracellular region is defined as: region != 0
	...
	FILE_LOG(logINFO) << " PARTICLES INITIALIZED IN BASIS " << endl;
		...
		while (i < ensemble.size()){
			...
			//cout << i << " " << a0 << " " << b0 << " " << c0 << " " << svec << "  " << inregion(svec) << endl;
			if( inregion(svec) != 0 ){
			ensemble[i] = Particle(scoords.x, scoords.y, scoords.z);
			}
			else{i--;}
	...
	void initializeInRegions(Ran & generator, vector<Particle> & ensemble, vector<int> & regions){
	
		Point scoords;
		double a0,b0,c0;
		int i = 0;
		FILE_LOG(logINFO) << " PARTICLES INITIALIZED IN SPECIFIED REGIONS " << endl;
		...
		while (i < ensemble.size()){
			...
			for (int j = 0; j < regions.size(); j++){
				...
				ensemble[i] = Particle(scoords.x, scoords.y, scoords.z); inside = true; break;
				}
			}
			if (inside == false){i--;}
			i++;
	...
	void correctBoundary(Vector3 & r){
		...
		if (inLatticeCell(r.x,r.y,r.z) == false){
			...
			if ( lcoords.x > a ) { lcoords.x = fmod(lcoords.x,a); }
			if ( lcoords.x < 0.0) { lcoords.x = fmod(lcoords.x,a) + a; }
		
			if ( lcoords.y > b ) { lcoords.y = fmod(lcoords.y,b); }
			if ( lcoords.y < 0.0) { lcoords.y = fmod(lcoords.y,b) + b; }
		
			if ( lcoords.z > c ) { lcoords.z = fmod(lcoords.z,c); }
			if ( lcoords.z < 0.0) { lcoords.z = fmod(lcoords.z,c) + c; }
	...
	bool inLatticeCell(double x, double y, double z){
		...
		if ( lcoords.x > a || lcoords.x < 0.0 || lcoords.y > b || lcoords.y < 0.0 || lcoords.z > c || lcoords.z < 0.0){
			return false;
		}
		return true;
	}
	...
	bool intersection(Vector3 & final_position, Vector3 & initial_position, Vector3 & normal, double & v) {

		vector< pair<double, Vector3 > > pos_v;
		FILE_LOG(logDEBUG4) << endl << " **************** LATTICE INTERSECTION METHOD ***************** " << endl;
		...
		Point initial(initial_position.x, initial_position.y, initial_position.z);
		Point final(final_position.x, final_position.y, final_position.z);
		Vector3 dr = final_position - initial_position;

		lmod(initial); lmod(final);
		Line line1(final.x - dr.x, final.y - dr.y, final.z - dr.z, final.x, final.y, final.z);
		Line line2(initial.x, initial.y, initial.z, initial.x + dr.x, initial.y + dr.y, initial.z + dr.z);
	...
	void addBasis(OBJECT o){

		basis.push_back(o);

	}

	void setLatticeVectors(double _a, double _b, double _c, Vector3  _ahat, Vector3  _bhat, Vector3 _chat){
	...
	double calcf(int points, int seed){
		...
		for (int i = 0; i < points; i++){
			...
			rand_simul = getsimulcoords(a*randomnumber.doub(), b*randomnumber.doub(), c*randomnumber.doub());
			temp[0] = rand_simul.x;temp[1] = rand_simul.y;temp[2] = rand_simul.z;
			
			for (int j = 0; j < basis.size(); j++){
			
				if ( basis[j].inside(temp) ) { points_inside++;}
