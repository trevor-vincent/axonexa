	...
	#ifndef OBJECTS_H_ //only allows one class declaration memory allotment to be made
	#define OBJECTS_H_
...
alignment in an array. */
	...
	private:
		...
		/* I'v added an extra piece of data in the vector structure
			called pad. This isn't part of the mathematics of vectors, and is purely there
			for performance. On many machines, four floating-point values sit more cleanly 
			in memory than three (memory is optimized for sets of four words), so noticeable
			speed-ups can be achieved by adding this padding.
		*/
		double pad;
	...
	class MultiSphere {
		...
		MultiSphere(){}
		MultiSphere(double, double, double,double, double, double, double, double, double, int, int);
		MultiSphere(Point & ,double, double, double, double, double, double, int, int);
		~MultiSphere(){};
