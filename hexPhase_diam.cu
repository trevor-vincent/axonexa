...
int main (){
  ...
  phaseAcquisitionStream<CosGFunc> pas(number_of_particles);
   ...
   int NOM = 2;
	...
	std::vector<real> radii(4); radii[0] = .0005; radii[1] = .001; radii[2] = .0025; radii[3] = .005;
  ...
  pas.runAcquisitionStreamLattice(cyls, lattice,  timestep, blocks, threads, 1, plan, numOfSMPerDevice); 
  pas.calcEveryADC();
