	...
	*/
	...
	for (int i = 0; i < 8; i++){
		...
		G[0] = 0.0;
		G[1] = sqrt(1000000.0/(GAMMA*GAMMA*grad_duration*grad_duration*(grad_duration + gspacings[i] - (grad_duration/3.0))));
		...
		measurements_x.push_back(PGSE(grad_duration,gspacings[i], timestep, G[1], echo_time, number_of_particles, xhat));
		measurements_y.push_back(PGSE(grad_duration,gspacings[i], timestep, G[1], echo_time, number_of_particles, yhat));
		measurements_z.push_back(PGSE(grad_duration,gspacings[i], timestep, G[1], echo_time, number_of_particles, zhat));
