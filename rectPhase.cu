...
cudaFuncSetCacheConfig( "_functionReduceAtom", cudaFuncCachePreferShared );
  ...
  for (int i = 0; i < numOfAcq; i++){
	  ...
	  real gradient_duration = echo_time*.25;
	  real gradient_spacing = echo_time - 2.0*gradient_duration;	
	  RectGrad qsc(i*1000000/((real)numOfAcq),gradient_duration,gradient_spacing, Vector3(1.0,0.0,0.0));
	  ...
	  phaseAcquisition<RectGrad> pa(1,number_of_timesteps,number_of_particles,i*time(NULL));
	  pa.addMeasurement(qsc);
	  pas.addAcquisition(pa); 
  ...
  pas.runAcquisition(1,cylinder,  sub, initialM,  timestep, blocks, threads, 14);
...
pas.runAcquisitionWC(1,cylinder,  sub, initialM,  timestep, blocks, threads);
 ...
 for (int i = 0; i < numOfAcq; i++){
   ...
   cout << Vector3( pas.getAcquisition(i).getMx()[0], pas.getAcquisition(i).getMy()[0], 0.0)*(1.0/number_of_particles);
  }
