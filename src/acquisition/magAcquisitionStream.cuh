...
template <class FieldType> class magAcquisitionStream {
  ...
  public:
    ...
    magAcquisition<FieldType> &getAcquisition(int num) { return acqSet[num]; }

    // put in Acquisition class
    template <class Basis>
    void runAcquisition(int num, Basis &basis, Vector3 &initialM, real timestep,
                        int blocks, int threads, int devNum, int numOfSM) {
        ...
        cudaScalar<SimuParams> dev_par(SimuParams(
            number_of_particles, steps, measurements, timestep,
            acqSet[num].getSeed(), initialM.x, initialM.y, initialM.z));
        ...
        updateWalkersMag<FieldType, Basis><<<blocks, threads>>>(
            ...
            dev_mx.getPointer(), dev_my.getPointer(), dev_mz.getPointer()
        ...
        dev_mx.sum(dev_total_mx, 1024, numOfSM, measurements,
                   number_of_particles, 0);
        dev_my.sum(dev_total_my, 1024, numOfSM, measurements,
                   number_of_particles, 0);
        dev_mz.sum(dev_total_mz, 1024, numOfSM, measurements,
                   number_of_particles, 0);
    ...
    template <class Basis>
    void runCPUAcquisition(int num, Basis &basis, Vector3 &initialM,
                           real timestep) {
        ...
        SimuParams pars(number_of_particles, steps, measurements, timestep,
                        acqSet[num].getSeed(), initialM.x, initialM.y,
                        initialM.z);

        srand(acqSet[num].getSeed());

        updateWalkersMagCPU<FieldType, Basis>(
            &pars, &basis, &acqSet[num].getFieldFunctors()[0], &mx[0], &my[0],
            &mz[0]);
    ...
    template <class Basis>
    void runAcquisitionStream(Basis &basis, Vector3 &initialM, real timestep,
        ...
        std::vector<std::vector<cudaScalar<SimuParams>>> dev_pars(
            numOfDevices, std::vector<cudaScalar<SimuParams>>(2));
        std::vector<std::vector<cudaVector<real>>> dev_mx(
            numOfDevices, std::vector<cudaVector<real>>(2));
        std::vector<std::vector<cudaVector<real>>> dev_my(
            numOfDevices, std::vector<cudaVector<real>>(2));
        std::vector<std::vector<cudaVector<real>>> dev_mz(
            numOfDevices, std::vector<cudaVector<real>>(2));
        std::vector<std::vector<cudaVector<real>>> dev_total_mx(
            numOfDevices, std::vector<cudaVector<real>>(2));
        std::vector<std::vector<cudaVector<real>>> dev_total_my(
            numOfDevices, std::vector<cudaVector<real>>(2));
        std::vector<std::vector<cudaVector<real>>> dev_total_mz(
            numOfDevices, std::vector<cudaVector<real>>(2));
        ...
        for (int devNum = 0; devNum < numOfDevices; devNum++) {
            ...
            for (int i = plan[devNum]; i < plan[devNum + 1]; i = i + 2) {
                ...
                host_pars[i] = SimuParams(
                    number_of_particles, steps, measurements, timestep,
                    acqSet[i].getSeed(), initialM.x, initialM.y, initialM.z);

                host_pars[i + 1] =
                    SimuParams(number_of_particles, steps1, measurements1,
                               timestep, acqSet[i + 1].getSeed(), initialM.x,
                               initialM.y, initialM.z);
                ...
                updateWalkersMag<<<blocks, threads, 0, streams[devNum][0]>>>(
                    dev_pars[devNum][0].getPointer(),
                    dev_basis[devNum][0].getPointer(),
                    dev_fields[devNum][0].getPointer(),
                    devStates[devNum][0].getPointer(),
                    dev_mx[devNum][0].getPointer(),
                    dev_my[devNum][0].getPointer(),
                    dev_mz[devNum][0].getPointer());
                updateWalkersMag<<<blocks, threads, 0, streams[devNum][1]>>>(
                    dev_pars[devNum][1].getPointer(),
                    dev_basis[devNum][1].getPointer(),
                    dev_fields[devNum][1].getPointer(),
                    devStates[devNum][1].getPointer(),
                    dev_mx[devNum][1].getPointer(),
                    dev_my[devNum][1].getPointer(),
                    dev_mz[devNum][1].getPointer());

                dev_mz[devNum][0].sum(dev_total_mz[devNum][0], 768, numOfSM,
                                      measurements, number_of_particles,
                                      streams[devNum][0]);
                dev_mz[devNum][1].sum(dev_total_mz[devNum][1], 768, numOfSM,
                                      measurements1, number_of_particles,
                                      streams[devNum][1]);
                dev_my[devNum][0].sum(dev_total_my[devNum][0], 768, numOfSM,
                                      measurements, number_of_particles,
                                      streams[devNum][0]);
                dev_my[devNum][1].sum(dev_total_my[devNum][1], 768, numOfSM,
                                      measurements1, number_of_particles,
                                      streams[devNum][1]);
                dev_mx[devNum][0].sum(dev_total_mx[devNum][0], 768, numOfSM,
                                      measurements, number_of_particles,
                                      streams[devNum][0]);
                dev_mx[devNum][1].sum(dev_total_mx[devNum][1], 768, numOfSM,
                                      measurements1, number_of_particles,
                                      streams[devNum][1]);

                host_mz[i].copyFromDevice(dev_total_mz[devNum][0],
                                          streams[devNum][0]);
                host_mz[i + 1].copyFromDevice(dev_total_mz[devNum][1],
                                              streams[devNum][1]);
    ...
    void addMagnetizationSet(vector<pinnedVector<real>> &host_mx,
                             ...
                             vector<pinnedVector<real>> &host_mz) {
        for (int i = 0; i < acqSet.size(); i++) {
