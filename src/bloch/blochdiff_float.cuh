...
__device__ void updateMagDORP(const Vector3 &M, const Vector3 &r,
    ...
    Vector3 k3 = bloch((M + k1 * (3.0f / 40.0f) + k2 * (9.0f / 40.0f)),
                       B(r, tn + .3 * h)) *
                 h;
    Vector3 k4 = bloch((M + k1 * (44.0f / 45.0f) + k2 * (-56.0f / 15.0f) +
                        k3 * (32.0f / 9.0f)),
                       B(r, tn + .8 * h)) *
                 h;
    Vector3 k5 =
        bloch((M + k1 * (19372.0f / 6561.0f) + k2 * (-25360.0f / 2187.0f) +
               k3 * (64448.0f / 6561.0f) + k4 * (-212.0f / 729)),
              B(r, tn + (8.0f / 9.0f) * h)) *
        h;
    Vector3 k6 = bloch((M + k1 * (9017.0f / 3168.0f) + k2 * (-355.0f / 33.0f) +
                        k3 * (-46732.0f / 5247.0f) + k4 * (49.0f / 176.0f) +
                        k5 * (-5103.0f / 18656.0f)),
                       B(r, tn + h)) *
                 h;
    Vector3 k7 = bloch((M + k1 * (35.0f / 384.0f) + k3 * (500.0f / 1113.0f) +
                        k4 * (125.0f / 192.0f) + k5 * (-2187.0f / 6784.0f) +
                        k6 * (11.0f / 84.0f)),
                       B(r, tn + h)) *
                 h;
    Mnew = M + k1 * (35.0f / 384.0f) + k3 * (500.0f / 1113.0f) +
           k4 * (125.0f / 192.0f) + k5 * (-2187.0f / 6784.0f) +
           k6 * (11.0f / 84.0f);
    Vector3 Mnew5 = M + k1 * (5179.0f / 57600.0f) + k3 * (7571.0f / 16695.0f) +
...
__device__ __host__ Vector3 updateMag_noT2_rotate(Vector3 M, Vector3 B,
    ...
    return (C + u * cos(-GAMMA * Bmag * timestep) +
            ((B * (1.0f / Bmag)) % u) * sin(-GAMMA * Bmag * timestep));
}
