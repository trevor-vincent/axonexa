...
class QSC_GRE {
  ...
  public:
    ...
    __device__ __host__ QSC_GRE(real _B0, real _G, real _gradientDuration,
    ...
    __host__ real getB() {
        return G * G * gradientDuration * gradientDuration * GAMMA * GAMMA *
               ((2.0 / 3.0) * gradientDuration + gradientSpacing);
    }
...
class QSC_PGSE {
  ...
  public:
    ...
    __device__ __host__ QSC_PGSE(real _B0, real _G, real _gradientDuration,
                                 real _gradientSpacing, real _rfDuration,
                                 real _dtmin, real _dtmax, real _TE) {
        B0 = _B0;
        G = _G;
        gradientDuration = _gradientDuration;
        gradientSpacing = _gradientSpacing;
    ...
    __device__ real getStepTimeMin(real time) {
        if (time >= TE / 2.0 && time < TE / 2.0 + rfDuration) {
            return dtmin;
        } else {
            return dtmax;
        }
...
class QSC_COS_GRE {
  ...
  public:
    ...
    __device__ __host__ QSC_COS_GRE(real _B0, real _G, real _gradientDuration,
                                    real _gradientSpacing, real _numOfPeriods) {
        B0 = _B0;
        G = _G;
        gradientDuration = _gradientDuration;
        gradientSpacing = _gradientSpacing;
        b = GAMMA * GAMMA * (1.0 / 4.0) * G * G * gradientDuration *
            gradientDuration * gradientDuration /
            (PI * PI * _numOfPeriods * _numOfPeriods);
    ...
    __device__ __host__ Vector3 operator()(Vector3 r, real time) const {
        ...
        if (time < gradientDuration) {
            ...
            return Vector3(0.0, G * ampMod(time) * r.z,
                           B0 + G * ampMod(time) * r.y);
            ...
            return Vector3(
                0.0,
                -G * ampMod(time - gradientSpacing - gradientDuration) * r.z,
                B0 - G * ampMod(time - gradientSpacing - gradientDuration) *
                         r.y);
            // return Vector3( 0.0, 0.0 , B0 - G*ampMod(time - gradientSpacing -
            // gradientDuration)*r.y);
    ...
    __host__ real getB() { return b; }

    __host__ real getFreq() { return 1.0 / period; }
};
...
class QSC_GRE_TRAP {
  ...
  public:
    ...
    __device__ __host__ QSC_GRE_TRAP(real _B0, real _G, real _gradientDuration,
                                     real _gradientSpacing,
                                     real _gradientRampTime) {
        B0 = _B0;
        G = _G;
        gradientDuration = _gradientDuration;
        gradientSpacing = _gradientSpacing;
    ...
    __host__ void printGz(real total_time, real timestep) {
        ...
        for (int i = 0; i < steps; i++) {
            std::cout << i * timestep << " "
                      << operator()(Vector3(0.0, 1.0, 0.0), i *timestep)
                      << std::endl;
        }
    }
};
...
class SIN_GRE {
  ...
  public:
    ...
    __device__ __host__ SIN_GRE(real _B0, Vector3 _G, real _gradientDuration,
                                real _gradientSpacing, real _period) {
        B0 = _B0;
        G = _G;
        gradientDuration = _gradientDuration;
        gradientSpacing = _gradientSpacing;
    ...
    __device__ __host__ Vector3 operator()(Vector3 r, real time) const {
        ...
        if (time < gradientDuration) {
            ...
            return Vector3(
                ...
                B0 - G * r *
                         sin(2.0 * PI *
                             (time - gradientSpacing - gradientDuration) /
                             period));
        }
...
class COS_GRE {
  ...
  public:
    ...
    __device__ __host__ COS_GRE(real _B0, Vector3 _G, real _gradientDuration,
        ...
        b = GAMMA * GAMMA * (1.0 / 4.0) * G.magnitude() * G.magnitude() *
            gradientDuration * gradientDuration * gradientDuration /
            (PI * PI * _numOfPeriods * _numOfPeriods);
    }
    __device__ __host__ COS_GRE(real _B0, real _G, Vector3 G_hat,
                                real _gradientDuration, real _gradientSpacing,
                                int _numOfPeriods) {
        B0 = _B0;
        G = G_hat * _G;
        gradientDuration = _gradientDuration;
        gradientSpacing = _gradientSpacing;
        period = gradientDuration / ((real)_numOfPeriods);
        b = GAMMA * GAMMA * (1.0 / 4.0) * _G * _G * gradientDuration *
            gradientDuration * gradientDuration /
            (PI * PI * _numOfPeriods * _numOfPeriods);
    }
    __device__ __host__ real getB() { return b; }
    __device__ __host__ real getFreq() { return 1.0 / period; }
    __device__ __host__ Vector3 operator()(Vector3 r, real time) const {
        if (time < gradientDuration) {
            return Vector3(0.0, 0.0,
                           B0 + G * r * cos(2.0 * PI * time / period));
        }
...
class RECT_GRE {
  ...
  public:
    ...
    __device__ __host__ RECT_GRE(real _B0, Vector3 _G, real _gradientDuration,
                                 real _gradientSpacing) {
        B0 = _B0;
        G = _G;
        gradientDuration = _gradientDuration;
...
class UCN_lowfield {
  ...
  public:
    ...
    __device__ __host__ UCN_lowfield(real _B0, real _G, real _rfDuration,
                                     real _dtmin, real _dtmax) {
        B0 = _B0;
        G = _G;
        rfDuration = _rfDuration;
        dtmin = _dtmin;
    ...
    __device__ __host__ Vector3 operator()(Vector3 r, real time) const {
        ...
        if (time < rfDuration) {
            return Vector3(B1 * cos(w * time) - .5 * G * r.x,
                           -B1 * sin(w * time) - .5 * G * r.y, G * r.z + B0);
        }
