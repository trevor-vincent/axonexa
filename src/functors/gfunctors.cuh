class RectGrad {
  ...
  public:
    ...
    __device__ __host__ RectGrad(real b, real gd, real gspac, Vector3 &ghat) {
        ...
        g_hat = ghat;
        gradient_strength =
            sqrt(b / (gradient_duration * gradient_duration * GAMMA * GAMMA *
                      ((2.0 / 3.0) * gradient_duration + gradient_spacing)));
    }
    ...
    __device__ __host__ RectGrad(real gstr, real gd, real gspac) {
        ...
        bvalue = gradient_strength * gradient_strength * gradient_duration *
                 gradient_duration * GAMMA * GAMMA *
                 ((2.0 / 3.0) * gradient_duration + gradient_spacing);
    }
    ...
    __device__ __host__ Vector3 operator()(real time) const {
        ...
        else if (time >= gradient_spacing + gradient_duration &&
                 time < 2.0 * gradient_duration + gradient_spacing) {
    ...
    __host__ real getB() { return bvalue; }
    ...
    __host__ real getG() { return gradient_strength; }

    __host__ real getTau() { return gradient_duration + gradient_spacing * .5; }
};
  ...
  public:
    ...
    __device__ __host__ Vector3 operator()(real time) const {
        ...
        if (time < gradientDuration) {
            ...
            return (gHat * -G) *
                   ampMod(time - gradientSpacing - gradientDuration);
        } else {
    ...
    __device__ __host__ real getG() { return G; }
};
...
class SinGFunc {
  ...
  public:
    ...
    __device__ __host__ SinGFunc(real _G, real _gradientDuration,
                                 real _gradientSpacing, int _numOfPeriods,
                                 Vector3 &_gHat) {
        ...
        G = _G;
        period = gradientDuration / ((real)_numOfPeriods);
        b = GAMMA * GAMMA * (3.0 / 4.0) * G * G * gradientDuration *
            gradientDuration * gradientDuration /
            (PI * PI * _numOfPeriods * _numOfPeriods);
    }
...
class ApodCosGFunc {
  ...
  public:
    ...
    __device__ __host__ ApodCosGFunc(real _G, real _gradientDuration,
                                     real _gradientSpacing, int _numOfPeriods,
                                     Vector3 &_gHat) {
        gradientDuration = _gradientDuration;
        gradientSpacing = _gradientSpacing;
...
class SWOGSEFunc {
  ...
  public:
    ...
    __device__ __host__ SWOGSEFunc(real _G, real _gradientDuration,
                                   real _gradientSpacing, int _numOfPeriods,
                                   Vector3 &_gHat) {
        ...
        double a1 =
            (1. - pow(-1., 2. * _numOfPeriods) + 4. * gradientDuration * v) /
            (4. * v);
        b = G * G * GAMMA * GAMMA *
            (gradientDuration / (6. * v * v) + gradientSpacing *
                                                   (gradientDuration - a1) *
                                                   (gradientDuration - a1));
    }
    __device__ __host__ Vector3 operator()(real time) const {
