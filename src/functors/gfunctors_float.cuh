    ...
    __device__ __host__ RectGrad(real b, real gd, real gspac, Vector3 &ghat) {
        ...
        gradient_strength =
            sqrt(b / (gradient_duration * gradient_duration * GAMMA * GAMMA *
                      ((2.0f / 3.0f) * gradient_duration + gradient_spacing)));
    }
    ...
    __device__ __host__ Vector3 operator()(real time) const {
        ...
        if (time < gradient_duration) {
            ...
            return g_hat * gradient_strength;
                 ...
                 time < 2.0f * gradient_duration + gradient_spacing) {
...
class CosGFunc {
  ...
  public:
    ...
    __device__ __host__ CosGFunc(real _G, real _gradientDuration,
        ...
        b = GAMMA * GAMMA * (1.0f / 4.0f) * G * G * gradientDuration *
    ...
    __device__ __host__ Vector3 operator()(real time) const {
        ...
        if (time < gradientDuration) {
            return (gHat * G) * ampMod(time);
                 ...
                 time < 2.0f * gradientDuration + gradientSpacing) {
            return (gHat * -G) *
