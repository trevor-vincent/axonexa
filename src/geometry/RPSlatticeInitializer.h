...
class RPSLatticeInitializer {
  ...
  public:
    template <class OBJECT> RPSLattice(int num, Lattice<OBJECT> &lattice) {
        lattice.basis.resize(num);
        lattice.setLatticeVectors(Vector3(1.0, 0.0, 0.0),
                                  Vector3(0.0, 1.0, 0.0),
                                  Vector3(0.0, 0.0, 1.0));
    }
    ...
    void uniformRadialDist(Lattice<OBJECT> &lattice, int seed,
    ...
    template <class OBJECT>
    void gaussianRadialDist(Lattice<OBJECT> &lattice, int seed, double mean,
                            double stddev) {
    ...
    template <class OBJECT>
    void gammaRadialDist(Lattice<OBJECT> &lattice, int seed, double alpha,
                         double beta) {
    ...
    template <class OBJECT> void correctEdges(Lattice<OBJECT> &lattice) {
                ...
                for (int i = -1; i <= 1; i++) {
                    for (int j = -1; j <= 1; j++) {
                        for (int k = -1; k <= 1; k++) {
                            ...
                            Point cnew = Point(ptemp.x + a * i, ptemp.y + b * j,
                                               ptemp.z + c * k);
                            temp.setCenter(cnew);
    ...
    template <class OBJECT> bool basisOverlap(OBJECT &o) {
    ...
    template <class OBJECT> bool objectOutside(OBJECT &o) {
        ...
        if (!inLattice(center + xhat * o.radius)) {
            return false;
        } else if (!inLattice(center - xhat * o.radius)) {
            return false;
        } else if (!inLattice(center + yhat * o.radius)) {
            return false;
        } else if (!inLattice(center - yhat * o.radius)) {
            return false;
        } else if (!inLattice(center + zhat * o.radius)) {
            return false;
        } else if (!inLattice(center - zhat * o.radius)) {
            return false;
        } else {
