...
void updateWalkersMagCPU(
    ...
    const SimuParams *par, const Basis *basis, const FieldType *B,
...

{
    ...
    for (int tid = 0; tid < par->number_of_particles; tid++) {
        ...
        for (int i = 0; i < par->measurements; i++) {
            ...
            // i*par->number_of_particles], My[tid +
            // i*par->number_of_particles], Mz[tid + i*par->number_of_particles]
            // );
        ...
        for (int i = 1; i < par->steps; i++) {
            ...
            phi = 2.0f * PI * unifRandCPP();
            theta = acos(2.0f * unifRandCPP() - 1);
            ...
            for (int j = 0; j < par->measurements; j++) {
                ...
                // j*par->number_of_particles], Mz[tid +
                // j*par->number_of_particles] );
            }
        }
...
void updateWalkersPhaseCPU(
    ...
    const SimuParamsPhase *par, const Basis *basis, const FieldType *G,
    real *phase
...

{
    ...
    for (int tid = 0; tid < par->number_of_particles; tid++) {
        ...
        for (int i = 1; i < par->steps; i++) {
            ...
            r += Vector3(stepLength * randomSign(), stepLength * randomSign(),
            ...
            for (int j = 0; j < par->measurements; j++) {
                ...
                kahanY = GAMMA * (G[j](par->timestep * i) * r) * par->timestep -
                         kahanC;
...
__host__ void boundaryNormalCPU(Vector3 &ri, Vector3 &r,
                                const real currentSpeed, const Basis *basis,
                                const real timestep) {
