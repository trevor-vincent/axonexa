...
__global__ void updateWalkersMag(const SimuParams *par, const Basis *basis,
...

{
    ...
    for (int i = 0; i < par->measurements; i++) {
        ...
        updateMagRK4(r, &Mx[tid + i * par->number_of_particles],
                     ...
                     &Mz[tid + i * par->number_of_particles], B[i], T1, T2, 0.,
                     par->timestep);
        ...
        updateMag_DORP(r, &Mx[tid + i * par->number_of_particles],
                       &My[tid + i * par->number_of_particles],
                       &Mz[tid + i * par->number_of_particles], B[i], 0.,
                       par->timestep);

#elif defined ROTATE_MULTISTEP

        updateMag_rotate_noT2_multistep(
            r, &Mx[tid + i * par->number_of_particles],
            &My[tid + i * par->number_of_particles],
            &Mz[tid + i * par->number_of_particles], B[i], 0., par->timestep);
    ...
    for (int i = 1; i < par->steps; i++) {
        ...
        for (int j = 0; j < par->measurements; j++) {
            ...
            updateMagRK4(r, &Mx[tid + j * par->number_of_particles],
                         ...
                         &Mz[tid + j * par->number_of_particles], B[j], T1, T2,
                         i, par->timestep);

#elif defined DORP_NORELAXATION

            updateMag_DORP(r, &Mx[tid + j * par->number_of_particles],
                           &My[tid + j * par->number_of_particles],
                           &Mz[tid + j * par->number_of_particles], B[j], i,
                           par->timestep);

#elif defined ROTATE_MULTISTEP

            updateMag_rotate_noT2_multistep(
                r, &Mx[tid + j * par->number_of_particles],
                &My[tid + j * par->number_of_particles],
                &Mz[tid + j * par->number_of_particles], B[j], i,
                par->timestep);
