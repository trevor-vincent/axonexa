...
class TannerStejkalSequence {
  ...
  public:
    TannerStejkalSequence(double _timestep, double _grad_duration,
                          double _grad_strength, double _echo_time,
                          double _grad_spacing, Vector3 _graddir,
    ...
    void updatePhase(std::vector<Particle> &ensemble, double time) {
        ...
        if (!doub_equal(gradmag, 0.0) && !(time > echo_time)) {
            ...
            for (int i = 0; i < phase.size(); i++) {
                ...
                // phase[i] +=
                // GAMMA*gradmag*(graddir*ensemble[i].unbounded_position)*timestep;
                phase[i] += GAMMA * gradmag *
                            (graddir * (ensemble[i].unbounded_position -
    ...
    void calcSignal(std::vector<Particle> &ensemble) {
        ...
        // FILE_LOG(logDEBUG3) << std::endl << "***********UPDATE SIGNAL
        // METHOD*********" << std::endl;
        ...
        for (int i = 0; i < phase.size(); i++) {
            ...
            // FILE_LOG(logDEBUG3) << "ensemble[i].T2factor = " <<
            // ensemble[i].T2factor << std::endl; FILE_LOG(logDEBUG3) <<
            // "total_tmv_x = " << total_tmv_x << std::endl; FILE_LOG(logDEBUG3)
            // << "total_tmv_y = " << total_tmv_y << std::endl;
        }

        signal = (sqrt(total_tmv_x * total_tmv_x + total_tmv_y * total_tmv_y)) /
                 (double)phase.size();
    }

    // Signal in a boxed region with initial and final defining the diagonal of
    // the box.
    void calcSignal(std::vector<Particle> &ensemble, Vector3 &initial,
                    Vector3 &final) {
    ...
    void calcSignal(std::vector<Particle> &ensemble, Lattice<OBJECT> &lattice,
                    int region) {
...
class PGSE : public TannerStejkalSequence {
  ...
  public:
    PGSE(double _grad_duration, double _grad_spacing, double _timestep,
         double _grad_strength, double _echo_time, int number_of_particles,
         Vector3 _graddir)
        : TannerStejkalSequence(_timestep, _grad_duration, _grad_strength,
                                _echo_time, _grad_spacing, _graddir,
                                number_of_particles) {
        diffusion_time = grad_duration + grad_spacing - (grad_duration / 3.0);
        b = GAMMA * GAMMA * grad_strength * grad_strength * grad_duration *
            grad_duration *
            (grad_duration + grad_spacing - (grad_duration / 3.0));
    }
    PGSE(){};
    ~PGSE(){};
    ...
    double gradient(double t) {
        ...
        else if (doub_gtoe(t, grad_spacing + grad_duration, tol) &&
                 t < grad_spacing + 2.0 * grad_duration) {
            return -grad_strength;
        }
...
class OGSE_SINE : public TannerStejkalSequence {
  ...
  public:
    OGSE_SINE(int _num_of_periods, double _grad_duration, double _grad_spacing,
              double _timestep, double _grad_strength, double _echo_time,
              int _number_of_particles, Vector3 _graddir)
        : TannerStejkalSequence(_timestep, _grad_duration, _grad_strength,
                                _echo_time, _grad_spacing, _graddir,
                                _number_of_particles) {

        period = grad_duration / ((double)_num_of_periods);
        b = GAMMA * GAMMA * (3.0 / 4.0) * grad_strength * grad_strength *
            grad_duration * grad_duration * grad_duration /
            (PI * PI * _num_of_periods * _num_of_periods);
    ...
    double gradient(double t) {
        ...
        if (t < grad_duration) {
            ...
            return -1.0 * grad_strength *
                   sin(2.0 * PI * (t - grad_spacing - grad_duration) / period);
...
class OGSE_COS : public TannerStejkalSequence {
  ...
  public:
    OGSE_COS(int _num_of_periods, double _grad_duration, double _grad_spacing,
             double _timestep, double _grad_strength, double _echo_time,
             int _number_of_particles, Vector3 _graddir)
        : TannerStejkalSequence(_timestep, _grad_duration, _grad_strength,
                                _echo_time, _grad_spacing, _graddir,
        ...
        period = grad_duration / ((double)_num_of_periods);
        b = GAMMA * GAMMA * (1.0 / 4.0) * grad_strength * grad_strength *
            grad_duration * grad_duration * grad_duration /
            (PI * PI * _num_of_periods * _num_of_periods);
    ...
    double gradient(double t) {
        ...
        if (t < grad_duration) {
            ...
            return -1.0 * grad_strength *
                   cos(2.0 * PI * (t - grad_spacing - grad_duration) / period);
