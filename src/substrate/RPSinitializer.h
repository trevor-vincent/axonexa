...
template <class Basis> class RPSLatticeInitializer {
  ...
  public:
    RPSLatticeInitializer(Lattice &_lat, int _basisType,
                          bool _verbose = false) {
    ...
    void uniformRadialDist(int seed, real max_radius, real min_radius) {
        ...
        for (int i = 0; i < basis.size(); i++) {
            deviate =
                (max_radius - min_radius) * randomnumber.doub() + min_radius;
            if (deviate > 0.0) {
                radii[i] = deviate;
            } else {
                i--;
    ...
    void gammaRadialDist(int seed, double alpha, double beta, real minimum,
                         real maximum) {
        ...
        for (int i = 0; i < basis.size(); i++) {
            deviate = gd.dev();
            if (deviate > minimum && deviate < maximum) {
                radii[i] = deviate;
            } else {
                i--;
            }
    ...
    bool uniformCenterDist(int seed, int triesMax) {
        ...
        for (int i = size - 1; i > -1; i--) {
            ...
            do {
                ...
                if (verbose)
                    std::cout << " Setting center " << i << " = " << center[i]
                              << std::endl;
                if (tries > triesMax * (size - i)) {
                    return false;
                }
                tries++;
                        ...
                        for (int j = -1; j <= 1; j++) {
                            for (int k = -1; k <= 1; k++) {
                                if (!(i == 0 && j == 0 && k == 0)) {
                                    ...
                                    Vector3 cnew = Vector3(ptemp.x + a * i,
                                                           ptemp.y + b * j,
                                                           ptemp.z + c * k);
                                    temp.setCenter(cnew);
                        ...
                        for (int j = -1; j <= 1; j++) {
                            if (!(i == 0 && j == 0)) {
                                ...
                                Vector3 cnew = Vector3(ptemp.x + a * i,
                                                       ptemp.y + b * j, 0.0);
                                temp.setCenter(cnew);
                                if (circleIntersectsLattice(temp.getCenter().x,
                                                            temp.getCenter().y,
                                                            temp.getRadius())) {
                                    basis.push_back(temp);
                                }
    ...
    // to make it work for spheres we must add a j,k and l (triple for loop) to
    // check for z overlaps
        ...
        for (int l = -1; l < 1; l++) {
            for (int m = -1; m < 1; m++) {
                ...
                for (int i = oi + 1; i < basis.size(); i++) {
                    ...
                    for (int j = -1; j < 1; j++) {
                        for (int k = -1; k < 1; k++) {
                            Vector3 center2 = basis[i].getCenter() +
                                              Vector3(a * j, b * k, 0.0);
    ...
    bool objectOutside(Basis &o) {
        ...
        if (!lat.inLatticeCell(r1)) {
            return true;
        } else if (!lat.inLatticeCell(r2)) {
            return true;
        } else if (!lat.inLatticeCell(r3)) {
            return true;
        } else if (!lat.inLatticeCell(r4)) {
            return true;
        } else if (!lat.inLatticeCell(r5) && basisType == 1) {
            return true;
        } else if (!lat.inLatticeCell(r6) && basisType == 1) {
            return true;
        } else {
            return false;
        }
    ...
    bool circleIntersectsLattice(real x, real y, real r) {
        ...
        if (dist2rectcen_x > (recthalfWidth + r)) {
            return false;
        }
        if (dist2rectcen_y > (recthalfHeight + r)) {
            return false;
        }

        if (dist2rectcen_x <= (recthalfWidth)) {
            return true;
        }
        if (dist2rectcen_y <= (recthalfHeight)) {
            return true;
        }

        real cornerDistance_sq = (dist2rectcen_x - recthalfWidth) *
                                     (dist2rectcen_x - recthalfWidth) +
                                 (dist2rectcen_y - recthalfHeight) *
                                     (dist2rectcen_y - recthalfHeight);
    ...
    real getVI() {
        ...
        for (int i = 0; i < basis.size(); i++) {
            ...
            for (int j = 0; j < radiiSoFar.size(); j++) {
                if (real_equal(tempRadius, radiiSoFar[j], EPSILON)) {
                    add = false;
                }
            }

            if (add)
                radiiSoFar.push_back(tempRadius);
        }
