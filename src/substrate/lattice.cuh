...
class Lattice {
  ...
  public:
    Lattice() {}
    Lattice(real _a, real _b, real _c, real _T2, real _T1, real _D,
            int _basisSize) {
        T2_lattice = _T2;
        T1_lattice = _T1;
        D_lattice = _D;
    ...
    __host__ __device__ real getA() { return a; }

    __host__ __device__ real getB() { return b; }

    __host__ __device__ real getC() { return c; }

    __device__ void initializeUniformly(Vector3 &r,
                                        curandState &localState) const {
    ...
    __device__ void initializeInRegion(const Basis *basis,
                                       curandState &localState, Vector3 &r,
                                       int region) const
    ...
    template <class Basis>
    __device__ __host__ double getD(const Basis *basis,
    ...
    template <class Basis>
    __device__ __host__ int inRegion(const Basis *basis,
                                     const Vector3 &r) const {
    ...
    __device__ __host__ bool inLatticeCell(const Vector3 &r) const {
        if (r.x > a || r.x < 0.0 || r.y > b || r.y < 0.0 || r.z > c ||
            r.z < 0.0) {
            return false;
        }
        return true;
    ...
    intersectionCheckPermeable(const Basis *basis, const Vector3 &ri,
                               const Vector3 &rf, real &v, Vector3 &n,
    ...
    template <class Basis>
    __device__ __host__ bool
    intersectionCheckImpermeable(const Basis *basis, const Vector3 &ri,
                                 const Vector3 &rf, real &v, Vector3 &n) const
    ...
    __device__ __host__ int getBasisSize() { return basisSize; }
